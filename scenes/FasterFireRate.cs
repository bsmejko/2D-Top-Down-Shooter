...
public partial class FasterFireRate : Area2D
{
    [Export] public float NewFireRate = 0.03f;
    ...
    public async void OnPickup(player player)
    {
        ...
        float originalRate = player.CurrentWeapon.WeaponName switch
        {
            "Pistol" => 0.2f,
            "Assault Rifle" => 0.1f,
            "Shotgun" => 0.3f,
        ...
        // Apply new fire rate
        player.CurrentWeapon.Firerate = NewFireRate;
        ...
        if (_currentIcon == _activeIcon && IsInstanceValid(player) && player.CurrentWeapon != null)
        {
            player.CurrentWeapon.Firerate = originalRate;
            RemovePowerUpIcon();
            _currentIcon = null;
        }
    }
    ...
    {
        float originalRate = aiAlly1.fireRate;
        aiAlly1.fireRate = NewFireRate;
        QueueFree();
        await Task.Delay((int)(BoostDuration * 1000));
        if (IsInstanceValid(aiAlly1)) aiAlly1.fireRate = originalRate;
    }
}